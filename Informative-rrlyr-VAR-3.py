...
DataPriors = DataPriors.sample(sample)
...
DataPriors = [dataTrain, dataTest]
DataPriors = pd.concat(DataPriors)
DataPriors = ut.downSampling(DataPriors)
...
for k in [3]:
    for Components in [4,6,8,10,12]:
        ...
        product = np.product(predProbs, axis=1)
        print("pass")

        V = np.diagflat(product)
        ...
        for size in[1000]:
            try:
                ...
                with pm.Model() as model:
                    priorsDict['Intercept'] = pm.Normal.dist(mu=intercept[0], sd=sdLogit[count]) #set 1: 0,1
                    count = count + 1
                ...
                f1Train, f1Test])
                pd.DataFrame(res).to_csv('Results/summaryMCMC/'+'Informative_rrlyr-VAR3-0702.csv')
                del dataTrain
                del dataTest
