...
fileTrain = 'data/BIASEDFATS/Train_rrlyr-2.csv'
fileTest =  'data/BIASEDFATS/Test_rrlyr-2.csv'
...
for k in [1,2,3]:
    for Components in [2,4,6,8,10,12]:
        for size in[500, 1000, 10000, 20000]:
            ...
            dataTrain = ut.Polinomial(dataTrain,k) #polynomial_kernel(dataTrain,degree=k) #
            ...
            dataTest = ut.Polinomial(dataTest,k) #polynomial_kernel(dataTrain,k) #
            ...
            pd.DataFrame(res).to_csv('Results/summaryMCMC/'+'MarginalLikelihood_flat_2.csv')
            del dataTrain
            del dataTest
